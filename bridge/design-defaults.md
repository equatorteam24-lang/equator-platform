...
- Кнопка бургера: 3 лінії (span), `position: relative; z-index: 10001`
- При відкритті: лінії анімуються в ХРЕСТИК (✕) — rotate 45deg / -45deg. Хрестик ОБОВ'ЯЗКОВИЙ для закриття меню.
- Відкрите меню:
  - `position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; height: 100dvh;`
  - `background: <колір теми>` — СУЦІЛЬНИЙ колір (hardcoded hex, НЕ CSS-змінна, НЕ прозорість, НЕ blur)
  - `z-index: 9999` (кнопка бургера/хрестика — `z-index: 10001`, вище за меню)
  - Посилання вертикально по центру, `gap: 28px`
- ⚠️ НЕ використовувати `inset: 0` — на деяких мобільних браузерах може не покривати весь viewport
- Блокування скролу: при відкритому меню додавати `document.body.style.overflow = 'hidden'` через useEffect
- При закритті: прибирати overflow hidden
- Колір фону меню має ЗБІГАТИСЯ з основним фоном сторінки або навбару, щоб було візуально цілісно
...
- ⚠️ КРИТИЧНО — ВИРІВНЮВАННЯ ПО ЛІВОМУ КРАЮ:
  Контент hero (заголовок, підзаголовок, кнопки) ЗАВЖДИ починається від лівого краю контейнера.
  
  **ЗАБОРОНЕНО на hero-content / контейнері тексту в hero:**
  - `justify-content: center`
  - `justify-content: space-between`
  - `align-items: center` (для горизонтального вирівнювання)
  - `margin: 0 auto` на текстовому блоці з обмеженим max-width
  - `text-align: center` на основному контенті
...
- ⚠️ У ВСІХ секціях де є фонове зображення/колір на весь екран і контент обмежений `max-width`:
  - Контент вирівнюється по ЛІВОМУ краю контейнера
  - НЕ використовувати `margin: 0 auto` на обмеженому блоці (тільки `margin-right: auto` або просто нічого — блок і так зліва)
  - НЕ використовувати `display: flex; justify-content: center/space-between` на контейнері hero тексту
  - Допоміжні елементи (scroll indicator, badge) — позиціонувати через `position: absolute`, а не через flex
  - Центрування допустиме ТІЛЬКИ якщо явно вказано в брифі
